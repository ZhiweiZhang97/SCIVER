...
class MPCNN(nn.Module):
    ...
    def _get_blocks_for_sentence(self, sent):
        ...
        for ws in self.filter_widths:
            ...
            block_b[ws] = {
                'max': F.max_pool1d(per_dim_conv_out_max,
                                    per_dim_conv_out_max.size(2)).contiguous().view(-1,
                                                                                    self.in_channels,
                                                                                    self.n_per_dim_filters),
                'min': F.max_pool1d(-1 * per_dim_conv_out_min,
                                    per_dim_conv_out_min.size(2)).contiguous().view(-1,
                                                                                    self.in_channels,
                                                                                    self.n_per_dim_filters)
            }
    ...
    def _horizontal_comparison(self,
                               sent1_block_a,
                               sent2_block_a,
                               pooling_types=('max', 'min', 'mean'),
    ...
    def _vertical_comparison(self,
                             sent1_block_a,
                             sent2_block_a,
                             sent1_block_b,
                             sent2_block_b,
                             holistic_pooling_types=('max', 'min', 'mean'),
                             per_dim_pooling_types=('max', 'min'),
